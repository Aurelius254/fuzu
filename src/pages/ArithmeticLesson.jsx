...
function NumberLineVisual({ number, revealed }) {
  const half = number / 2;
  const markers = [0, half, number];
  return (
    <div style={{ padding: "24px 16px" }}>
      <div style={{ position: "relative", height: 60 }}>
        <div style={{ position: "absolute", top: 28, left: 20, right: 20, height: 3, background: "#2a2a2a", borderRadius: 2 }} />
        {markers.map((val, i) => {
      ...
      <div style={{ textAlign: "center", color: "#555", fontSize: 12, marginTop: 8 }}>Half is exactly in the middle</div>
    </div>
  );
}

function DotsVisual({ number, revealed }) {
  const half = number / 2;
      ...
      <div style={{ display: "flex", gap: 20, justifyContent: "center", alignItems: "flex-start" }}>
        {[0, 1].map((group) => (
          <div key={group} style={{ textAlign: "center" }}>
            ...
            <div style={{ color: "#fff", fontWeight: 900, fontSize: 22, marginTop: 2 }}>{revealed ? half : "?"}</div>
          </div>
        ))}
      </div>
    </div>
